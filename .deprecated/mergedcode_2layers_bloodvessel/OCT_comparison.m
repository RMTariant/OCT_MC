...
maxDepth = 1.07; %%%%CHOOSE VALUE%%%%
%Compress image for refractive index
n_cor = 1.37; %%%%CHOOSE VALUE%%%%
...
ix = find(and(DetL < L_threshold,DetStot/2 < maxDepth));
...
%% Check the position of photons

zaxis = 0:0.001:0.7;
OCTmap = zeros(length(zaxis),Ndetectors);
for n = 1:Ndetectors
    nPhoton(n) = length(S2(1,ID == n));
    intPhoton(n) = sum(W(ID == n).*L(ID == n));
end
figure
plot(nPhoton)
figure
...
S = sum(S2,1);
for n = 1:Ndetectors
    for m = 1:length(zaxis)-1        
        select = and(and(S >= zaxis(m),S < zaxis(m+1)),ID == n);        
        OCTmap(m,n) = sum(W(select).*L(select));
    end
    n
end
figure
