...
int main(int argc, const char * argv[]) {
	...
	float	a_coef;         /* Biasing coefficient of the importance sampling */
	float	p;          	/* Probability of additional bias scattering */
	float zsurf;			/* Position of air/skin surface */
	
	/* detector parameters */
	float	N_det;          	/* Number of detectors/Number of Alines */
	double	span_det;          	/* Width of the Bscan. Centered at 0. */
	double	cos_accept;         /* Cosine of accepte angle aperture */
	double	det_radius;         /* Radius of of one detector region */
	...
	float 	mrv[Ntiss];              // anisotropy of scattering
	...
	// run parameters
		...
		fgets(buf, 32, fid);
		sscanf(buf, "%f", &Nphotons); // Total number of launched photon if time_min not used
		fgets(buf, 32, fid);
		sscanf(buf, "%f", &a_coef); // desired time duration of run [min]
		fgets(buf, 32, fid);
		sscanf(buf, "%f", &p); // probability of additional bias scattering
		fgets(buf, 32, fid);
		sscanf(buf, "%f", &N_det); // number of Alines
		fgets(buf, 32, fid);
		...
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &zsurf);  // position of air/skin surface
		...
		for (i=1; i<=Nt; i++) {
			...
			fgets(buf, 32, fid);
			sscanf(buf, "%f", &mrv[i]);		// anisotropy of scatter [dimensionless]
		}    
    fclose(fid);
