...
%File names. List all the names of the files
myname{1} = 'attenuationtest'; %%%%CHOOSE VALUE%%%%
myname{2} = 'attenuationtest2'; %%%%CHOOSE VALUE%%%%
%myname{3} = 'attenuationtest3'; %%%%CHOOSE VALUE%%%%
%myname{4} = 'attenuationtest4'; %%%%CHOOSE VALUE%%%%
%Number of photon loaded at the same time
nph = 40000; %%%%CHOOSE VALUE%%%%
%Limit the total number of loaded photons
maxnph = 80000000000000;
...
for n = 1:length(myname)
% Load header file
filename = sprintf('%s_H.mci',myname{n});
...
% Load path lengths of detected photons DetS
filename = sprintf('%s_DetS2.bin',myname{n});
disp(['loading ' filename])
tic
    fid = fopen(filename, 'rb');
...
filename = sprintf('%s_DetW.bin',myname{n});
disp(['loading ' filename])
tic
    fid = fopen(filename, 'rb');
...
filename = sprintf('%s_DetL.bin',myname{n});
disp(['loading ' filename])
tic
    fid = fopen(filename, 'rb');
...
filename = sprintf('%s_DetID.bin',myname{n});
disp(['loading ' filename])
tic
    fid = fopen(filename, 'rb');
    DetID = fread(fid, 'int');
...
filename = sprintf('%s_DetE.bin',myname{n});
disp(['loading ' filename])
tic
    fid = fopen(filename, 'rb');
