...
%Adding noise
noise1 = raylrnd(noise_amp/1.253*ones(size(s_sample2))) .* exp(1i.*2.*pi.*rand(size(s_sample2)));
noise2 = raylrnd(noise_amp/1.253*ones(size(s_sample2))) .* exp(1i.*2.*pi.*rand(size(s_sample2)));
...
I2 = (abs(s_sample2 + ref_amp + noise1)).^2 - (abs(s_sample2 - ref_amp + noise2) ).^2;
